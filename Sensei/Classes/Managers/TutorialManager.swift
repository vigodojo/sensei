...
class TutorialManager {
    ...
    func delayForCurrentStep() -> Float {
        if !TutorialManager.sharedInstance.completed {
            if let currentStep = TutorialManager.sharedInstance.currentStep {
                if let prevStep = TutorialManager.sharedInstance.prevTutorialStep {
                    let delayBefore = (prevStep.text.characters.count == 0 || currentStep.delayBefore == 0) ? currentStep.delayBefore : Float(prevStep.text.characters.count) * 0.03
                    return  Float(delayBefore)
                }
                return Float(currentStep.delayBefore)
            }
        } else {
